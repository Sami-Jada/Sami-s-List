...
export class OrdersService {
   ...
   * Auto-creates user if phone doesn't exist, creates address, and places order
   */
  async createGuestOrder(createGuestOrderDto: CreateGuestOrderDto) {
    const { phone, address, tankQuantity, paymentMethod } = createGuestOrderDto;

    // Find or create user by phone (guest users have empty name)
    let user = await this.usersService.findByPhone(phone);
    
    if (!user) {
      // Create guest user (name = empty string indicates guest)
      const newUser = await this.usersService.create({
        phone,
        name: '', // Empty name indicates guest user
      });
      user = await this.usersService.findById(newUser.id);
      this.logger.log(`Guest user created: ${user.id} (${phone})`);
    }
    ...
    const createdAddress = await this.addressesService.create(
      ...
      {
        ...address,
        isDefault: true, // Set as default for guest users
      },
    );
    ...
    this.logger.log(
      `Guest order created: ${order.orderNumber} (${order.id}) by phone ${phone}`,
    );

    // Generate temporary token (1 hour expiry) for viewing order status
    const temporaryToken = await this.tokenService.generateTemporaryToken(
      user.id,
      phone,
    );
    ...
    return {
      ...
      accessToken: temporaryToken, // Short-lived token for guest order tracking
      isGuest: user.name === '', // Indicate if this is a guest user
    };
  }
